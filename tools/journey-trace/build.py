...
MAPPING = {
    ...
    "g2_1": m("implemented", "console", "#/onboard", "The reference's six-field identity form (legal name, country, work email, contact person, kind, sector) as a desktop console frame — Registration · 1 of 4 with the Register/Terms/Certificates/Done rail; registration documents deliberately not asked, per the reference's own callout → POST /v1/organisations. Gap kept honest: the Party schema is closed (additionalProperties:false), so country/kind/sector/contact person are held client-side for the flow and do not persist server-side"),
