...
MAPPING = {
    ...
    "p1_2": m("compressed", "console", "#/people", "The reference's frame at the rail's People & roles entry, listing every grant GET /v1/authorizations reports for the organisation with its grantor and grant date. Compressed: the invited/active distinction, the vacant-role line and the invitation itself need a role-grant write path and a notification service, and the screen says so rather than drawing states it cannot read"),
    "p1_3": m("illustrative", "console", "#/projects/new", "The reference's three-field frame (project name, coverage, named Configurator) with its 'not decided here' callout verbatim, and nothing to submit to: creating a project and naming its Configurator has no write path from this door yet. Labelled on its face — a named owner who never agreed is the failure the handover exists to prevent, so no local state pretends one exists"),
    ...
    "p2_10": m("illustrative", "console", "#/support", "The reference's frame and its 'what moved, and why' callout verbatim; no support owner can be named because there is nothing to write it to, and a support contact this console invented would send a worker to nobody"),
...
DESIGN_SCREENS = [
     ...
     **m("implemented", "console", "#/", "The console door: eSignet as the primary way in (the same server-side flow the worker door uses), the instance-configured demo personas below a divider, and NO role selector anywhere — a role is granted in the registry and read back, never self-declared. The persona block probes the mock issuer's discovery document and is absent where no mock provider answers")},
     ...
     **m("compressed", "console", "#/people", "The guard renders at the same rail entry the Org Admin uses: the Configurator sees the boundary stated, the organisation that can grant the role with its real contact route, and every grant with its grantor and date, read only. Compressed: the role a Configurator would need is named as 'Org Admin' in copy because role-to-function mapping is L2 and unbuilt, so nothing derives it from a permit")},
]
