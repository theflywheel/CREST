...
MAPPING = {
    ...
    "g1_5": m("implemented", "console", "#/instance/invite", "The reference's five fields, writing for real: #185 is settled (option (b), ruled 2026-09-07) — the instance-level invitation is a party record plus a claim code. POST /v1/organisations from the operator's session creates the organisation UNBOUND with its APPLIED registration and mints a one-time code; the named signatory claims it with their own login (POST /v1/party-invitations/claim), binding only their own subject, so #102 is not widened. The invitation addresses a record, not a person, which is how it needs no prior Party. The button is the reference's 'Send invitation': sending writes the record and mints the code, and the link is shown once with a copy control for the operator to share themselves — no channel carries it (#150), and the screen says so. No self-approval: the registry custodian still decides"),
    ...
    "f1_2": m("implemented", "console", "#/rateowner", "Rate-owner assignment: one current owner, every assignment kept as history; the Org Admin assigns"),
    "f1_3": m("implemented", "console", "#/rate", "Authoring: the assigned owner prices a unit somebody else defined — the unit is read-only by construction"),
    "f1_4": m("implemented", "console", "#/ratepublish", "Publication as a new version naming what it supersedes. No edit affordance exists, and the walk asserts its absence"),
    "f1_5": m("implemented", "console", "#/ratestanding", "The derived standing — not-configured / configured-not-live / live — as a real screen state, never stored"),
