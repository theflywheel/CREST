-- eSignet and mock-identity databases.
--
-- Vendored from mosip/esignet v1.8.0 docker-compose/init.sql, with two changes:
-- the databases are owned by the CREST role rather than the superuser, and the
-- schema objects are created by that same role. Nothing in this stack connects
-- as `postgres`; a payments-adjacent system that runs as a superuser has no
-- answer to "what could this service have done".
--
-- Regenerating: fetch that file, drop its CREATE DATABASE block, and replace
-- `OWNER TO postgres` with the role below. Keep the version in step with
-- ESIGNET_IMAGE in .env.example — the schema and the jar move together.

CREATE DATABASE mosip_esignet OWNER crest ENCODING 'UTF8';
CREATE DATABASE mosip_mockidentitysystem OWNER crest ENCODING 'UTF8';

\c mosip_esignet crest

DROP SCHEMA IF EXISTS esignet CASCADE;
CREATE SCHEMA esignet;
ALTER SCHEMA esignet OWNER TO crest;
