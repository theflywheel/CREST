...
-- (docker-compose/docker-compose-injistack/mimoto_init.sql), with the database
-- and schema owned by the CREST role rather than by the superuser.
--
-- Mimoto is the wallet's backend-for-frontend: it holds the OIDC client the
-- browser wallet authenticates through, and the tokens that come back. Its own
-- database for the same reason as the others — nothing here should be reachable
-- from a CREST migration.

CREATE DATABASE inji_mimoto OWNER crest ENCODING 'UTF8';

\c inji_mimoto crest

DROP SCHEMA IF EXISTS mimoto CASCADE;
CREATE SCHEMA mimoto;
ALTER SCHEMA mimoto OWNER TO crest;
