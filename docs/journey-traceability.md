...
| Screen | Stage | Reference title | Status | Surface | Evidence / gap |
|---|---|---|---|---|---|
| `g2_1` | Register | Six fields, and one of them decides everything after | **implemented** | console `#/onboard` | Six-field identity form (name, kind, sector, registration number, contact name, contact email) → POST /v1/organisations. Gap kept honest: the Party schema is closed (additionalProperties:false), so kind/sector/regNo/contact name are held client-side for the flow and do not persist server-side |
| `g2_4` | Done | Listed, approved, connected — and still on nobody’s project | **implemented** | console `#/onboard/status` | GET /v1/organisations/{id}/registration: APPROVED with decider and terms version; 'on nobody's project' is true — no project membership exists |
