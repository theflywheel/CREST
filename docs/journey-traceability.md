...
In scope today (J3 — `p1_*`, `p2_*`, plus the design screens `n1`–`n5`): **29** screens — **7** asserted, **0** quarantined, **22** skipped with a reason.
...
| `p2_10` | Support | Support belongs to the project, not the platform | **illustrative** | console `#/support` | skipped (illustrative) | The reference's frame and its 'what moved, and why' callout verbatim; no support owner can be named because there is nothing to write it to, and a support contact this console invented would send a worker to nobody |
