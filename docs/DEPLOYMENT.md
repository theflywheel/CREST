...
**The demo fleet (2026-08-25).** All seven services, the three mocks, a seeding *job* (`crest-seed` — no resident deployment since 2026-08-28: the seeded world lives in Postgres, so the service exists only while a seed is running; `railway up --service crest-seed --ci --detach` runs one, `railway down --service crest-seed -y` removes it after) and one public door now run on Railway. Only `crest-web` has a public domain — https://crest-web-production.up.railway.app — an nginx that serves `apps/web` and proxies `/api/<service>/` over private networking, refusing `/internal/*` at the door (the §16 service-identity fence). The demo fleet runs `CREST_ENV=railway` with a driveable clock **inside the fence** (start 2026-03-01), the mock OIDC issuer, and fresh salts; it is a demo of the journeys, not the hardened deployment, and the two must not be conflated: the hardened path is still the matrix below.
