...
| `CREST_SERVICE_TOKEN` | The shared service-boundary token, at least 32 bytes, the **same value** on `crest-core` and `crest-payments` (the sender sets `X-CREST-Service-Token` from it; the receiver's boundary compares against it) — `crest-seed` used to hold it too and was deleted on 2026-09-09 (#155 phase 4). Railway service variables only |
