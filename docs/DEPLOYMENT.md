...
**The wallet in that walk is a script, not Inji Web — and why.** `crest-mimoto` now runs Mimoto 0.20.0 (#230), which carries the wallet-side OpenID4VP routes (`/wallets/{id}/presentations`, `/wallets/{id}/verifiers`) and a `mimoto-trusted-verifiers.json` naming the verify service's DID client id and `…/v1/verify/vp-submission/direct-post`. Inji Web (guest) downloads a CREST Work Event credential through Mimoto's OpenID4VCI, proven end to end on 2026-09-10 (Grace, eSignet PIN → "Success"). What it does not do from the browser is *present*: Inji Web serves its OpenID4VP `/authorize` flow only to a persistent, Google-authenticated wallet, because the guest flow hands the credential back as a downloaded PDF rather than storing it server-side to select for a verifier. That login is a MOSIP Inji Web UX requirement, not a CREST wiring gap — `GOOGLE_OAUTH_CLIENT_ID` is set on `crest-mimoto` — so the presentation half is still driven by `make openid4vp-present` against the same live request. `MOSIP_INJI_WEB_URL` is set on `crest-mimoto` so Mimoto redirects the browser back to the wallet after a share.
