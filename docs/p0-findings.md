...
| R5 | ~~DeDi-node needs Go 1.25; CREST is on 1.24~~ **Overtaken by events.** CREST moved to Go 1.25 when `pgx/v5` required it, so the version gap is gone | None either way. The conclusion it supported still stands for a better reason: separate modules with HTTP between them, because a transparency log CREST could vendor is a transparency log CREST could accidentally rewrite. |
...
| C11 — **resolved 2026-09-11 (#203).** | The Certify door (`crest-certify` nginx fronting the Java service, the way `crest-esignet-ui` fronts eSignet) serves the metadata at the advertised issuer; the servlet-path workaround is removed from Mimoto's config, and `make certify-issue` discovers the issuer at the bare host. Upstream: [mosip/inji-certify — credential_issuer/metadata path mismatch](docs/upstream/certify-wellknown-path-mismatch.md) (draft, to file). |
...
| C15 — **filed upstream (#203).** | Upstream: [mosip/inji-certify — display[0].logo required but undocumented](docs/upstream/certify-display-logo-required.md) (draft, to file). CREST's config carries a logo, so it is not blocked; the report is so a third-party issuer is not surprised. |
...
**The wallet download through Inji Web against the deployed stack was walked on 2026-09-10** (see [#66](../../issues/66) and `docs/DEPLOYMENT.md` "The wallet leg on Railway"): a real eSignet login from the wallet, Certify issuing from live work events, the card as a PDF, and Inji Verify answering valid. **Still open on #1:** the *physical* leg — a printed card scanned on a real device with its radios off. A container asserting it has no network is stronger evidence than a mock and weaker evidence than a phone, and the gap between them is exactly where a QR that is too dense to scan, or a card nobody can hold at the right distance, would show up. That needs a person, a printer and a phone.
...
**The id_token carries no `individual_id`.** The subject is the only identifier the RP receives (`sub`, `aud`, `acr`, `auth_time`, `iss`, `nonce`, `at_hash`), which is the shape W8 wants; E2 remains the thing to keep out of the registration.

**E2 — `claims_supported` includes `individual_id`.** The claim set is `name, address, gender, birthdate, picture, email, phone_number, individual_id, phone_number_verified, registration_type, updated_at`. `individual_id` is a national identifier: **requesting it would put CREST one careless persist away from breaking W8.** The scope-to-claim mapping is configuration, so this belongs in the relying-party registration as an explicit exclusion, not in a code review later. §4.1 should name the claims CREST requests and say plainly that `individual_id` is not one of them.
...
What remains of [#66](../../issues/66) is the physical leg — printed, scanned, radios off — which no amount of tooling can stand in for. The held leg (a browser wallet, a real login, the card in hand as a PDF, the verifier answering valid) was walked on the deployed fleet on 2026-09-10.
