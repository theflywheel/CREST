...
| **Mock SMS gateway** | Local channel testing | A real gateway is a paid external. The mock is ours (`tools/mocks/`), tiny, and lets the harness assert "the worker was notified" (W5–W6) without a phone. |
...
| `pkg/*` | Shared Go: ID/DID generation, JSON Schema validation, DeDi/Certify/eSignet clients, audit, outbox, HTTP scaffolding. |
...
| `demos/` | Recorded journey walkthrough videos (J1–J12), regenerated by `tests/e2e-apps/journeys.mjs` against a story-seeded **local** stack (`make apps-up`) — the deployed fleet has neither the seeder nor the dev issuer since 2026-09-09, #155 phase 4. Git-ignored — a demo is an artefact of the code, not a source of it. |
