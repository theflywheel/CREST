...
The canonical table, with the genericity note for each row, is §2 of the blueprint (the Infrastructure Blueprint, §2, `docs/crest-infrastructure-blueprint.html` in the repository); the JSON Schemas in `schemas/` are the source of truth for every field.
