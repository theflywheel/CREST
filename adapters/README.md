...
Add the package and one entry to [`builtin/builtin.go`](builtin/builtin.go), the one registration point every evidence-parsing service composes its registry from:

```go
func Plugins() []adapters.Plugin {
    return []adapters.Plugin{
        csv.Plugin(),
        example.Plugin(), // adapters.Plugin{Ref: example.Version, New: func() adapters.Adapter { return example.Adapter{} }}
    }
}
```

The registry validates the list at start-up — a duplicate, unversioned or mis-declared entry stops the service rather than parsing evidence wrongly — and lists what it holds at `GET /v1/adapters`.

A source is registered with the exact `adapterRef` it will be parsed by (`POST /v1/sources`), and that registration is what selects the parser: a batch is submitted against the source's `systemRef`, and the request may name `adapterRef` only to confirm it. Parser selection is deployment configuration, never a choice the payload makes.
