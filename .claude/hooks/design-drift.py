...
AREAS = [
    ...
    ("confirm", "§11 worker invariants",
     "all four T=7 exits release payment (W5–W6); no claim goes ACTIVE unnotified (W5–W6)"),
    ...
    ("verif", "§6 strength derivation, §9 consent",
     "tier is derived at query time, never stored; refusals are values, not errors (W7, W8)"),
    ("identity", "§4.1 identity providers",
     "pairwise reference and salted hash only — never a raw ID or biometric (W8)"),
    ("match", "§8 evidence ingestion",
     "probable matches hold; merges_without_confirmation stays 0 (W4)"),
