const CACHE = "crest-verify-shell-v1";

self.addEventListener("install", (event) => {
  event.waitUntil(
    caches.open(CACHE).then(async (cache) => {
      const response = await fetch("./index.html");
      if (!response.ok) throw new Error("verify shell unavailable");
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches.keys().then((keys) => Promise.all(
      keys.filter((key) => key.startsWith("crest-verify-shell-") && key !== CACHE).map((key) => caches.delete(key)),
